...
import type { ReviewMode, TtxItem, TtxSession, VocabItem, NativeNote } from '@/contracts/models';
...
const SessionPlayerModal: React.FC<SessionPlayerModalProps> = ({
  ...
  const { reset: resetMiniChat } = miniChat;
  ...
  const handleMiniChatSend = () => {
    ...
    miniChat.appendUserMessage(miniChatInput);
    miniChat.setInput('');
        ...
        <View style={styles.modalContent}>
          {reviewSummary ? (
            <View style={styles.reviewWrapper}>
              <ReviewCard
                summary={{
                  accuracy: reviewSummary.accuracy ?? 0,
                  avgTimeSeconds:
                    reviewSummary.durationsSeconds.reduce((acc, dur) => acc + dur, 0) /
                    Math.max(1, reviewSummary.durationsSeconds.length),
                  strengths: [],
                  focusAreas: reviewSummary.recommendedActions?.map(text => ({
                    prompt: text,
                    insight: '',
                    score: 0,
                  })) ?? [],
...
type AnalysisCardProps = {
  ...
  onAddNote: () => void;
  onFlag: () => void;
  miniChat: ReturnType<typeof useMiniChat>;
  onMiniChatSend: () => void;
