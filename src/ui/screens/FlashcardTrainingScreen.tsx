...
const FlashcardPlayerBody: React.FC<FlashcardPlayerBodyProps> = ({
  ...
  const outcomes = useMemo(() => {
    // Count every swipe ever recorded in the session so counters reflect total attempts.
    return cards.reduce(
      (acc, item) => {
  ...
  const applySrsUpdate = async (currentCard: FtxCard, outcome: FlashcardOutcome) => {
  ...
  const restartSessionWithCards = async (cardsToUse: FtxCard[]) => {
    const resetCards = cardsToUse.map(card => ({ ...card, history: [] }));
  ...
  const finalizeSession = async () => {
    const latest = cards
      ...
      .filter((entry): entry is FlashcardHistory => Boolean(entry));
    ...
    const recap = {
      ...
      srsQueue: Array.from(srsDueRef.current.entries()).map(([vocabId, dueAt]) => ({
        vocabId,
        dueAt,
      })),
    };
  ...
  const stats = useMemo(() => {
    // Use outcomes which is always calculated from fresh session data
    return {
      accuracy: outcomes.correct / Math.max(1, outcomes.correct + outcomes.incorrect),
      correctCount: outcomes.correct,
      incorrectCount: outcomes.incorrect,
      flaggedCardIds: cards.filter(item => item.isFlagged).map(item => item.cardId),
      srsQueue: [],
    };
  }, [outcomes, cards]);
            ...
            <View style={styles.summaryActions}>
              <Pressable
                style={[
                  ...
                  completedIncorrect === 0 && { opacity: 0.5 },
                ]}
              ...
              >
                <Text style={[styles.secondaryButtonLabel, { color: colors.accent }]}>Review all</Text>
              </Pressable>
              <Pressable style={[styles.secondaryButton, styles.fullWidthButton]} onPress={handleExitActivity}>
            ...
            <View style={styles.cardMetaRow}>
              ...
              <Pressable onPress={handleToggleFlag} style={styles.flagButton}>
                <Text
                  style={[
                    styles.flagLabel,
                    { color: card?.isFlagged ? colors.accent : colors.textSecondary },
                  ]}
                >
                  ★
                </Text>
